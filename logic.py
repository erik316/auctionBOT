...
class DatabaseManager:
    ...
    def get_prize_img(self, prize_id):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute("SELECT img FROM prizes WHERE prize_id = ?", (prize_id,))
    ...
    def get_rating(self):
        ...
        with conn:
            ...
            cur.execute('''SELECT users.user_name, COUNT(winners.prize_id) AS count_prize FROM winners 
                            INNER JOIN users ON users.user_id = winners.user_id GROUP BY winners.user_id ORDER BY count_prize LIMIT 10''')
            return cur.fetchall()
